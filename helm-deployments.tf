...
module "code_llama" {
  ...
  pre_deployment_commands   = ["export PUBLIC_IP=$(kubectl get svc -A -l app.kubernetes.io/name=ingress-nginx  -o json | jq -r '.items[] | select(.spec.type == \"LoadBalancer\") | .status.loadBalancer.ingress[].ip')"]
  deployment_extra_args = [
    #at this moment exposed via ingress and also as a sevice
    "--set ingress.hosts[0]=codellama.$${PUBLIC_IP}.nip.io",
...
module "cuopt" {
  ...
  # helm_chart_name               = "cuopt-24.03.00"
  namespace                     = "default"
  # helm_repository_url           = "https://helm.ngc.nvidia.com/nvidia/charts/"
  pre_deployment_commands       = ["kubectl create secret --namespace default docker-registry ngc-docker-reg-secret --docker-server=nvcr.io --docker-username='$oauthtoken' --docker-password='${var.NGC_API_KEY}'"]
  deployment_extra_args = []
  post_deployment_commands = []
  helm_template_values_override = templatefile(
    "${path.root}/helm-values-templates/cuopt-values.yaml.tpl",
    {}
  )
  helm_user_values_override = try(base64decode(var.nim_user_values_override), var.nim_user_values_override)
